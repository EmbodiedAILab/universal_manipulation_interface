    ...
    for session in session_dir:
        ...
        with ExifToolHelper() as et:
            for mp4_path in input_mp4_paths:
                ...
                pose_path = mp4_path.parent.joinpath(mp4_path.stem.replace('video', 'pose')+'.csv')
                out_trajectory_path= this_out_dir.joinpath('camera_trajectory.csv')
                shutil.move(pose_path, out_trajectory_path)
                gripper_path = mp4_path.parent.joinpath(mp4_path.stem.replace('video', 'gripper') + '.csv')
                out_trajectory_path = this_out_dir.joinpath('gripper_width.csv')
                shutil.move(gripper_path, out_trajectory_path)
