...
from umi.sim_world.sim_vacuum_controller import SimVacuumController
...
from umi.common.interpolation_util import get_interp1d, PoseInterpolator, get_step_interp1d


class BimanualR1SimEnv:
    def __init__(self, 
            # required params
            output_dir,
        ...
        camera_serial_numbers = [camera['serial_number'] for camera in cameras_config]
        ...
        vacuums: List[SimVacuumController] = list()
        ...
        for vc in vacuums_config:
            this_vacuum = SimVacuumController(
                shm_manager=shm_manager,
                frequency=50,
            )
            vacuums.append(this_vacuum)
        ...
        self.vacuums_config = vacuums_config
    ...
    def is_ready(self):
        ...
        for vacuum in self.vacuums:
            ready_flag = ready_flag and vacuum.is_ready
        return ready_flag
    ...
    def get_obs(self) -> dict:
        ...
        for vacuum in self.vacuums:
            last_vacuums_data.append(vacuum.get_all_state())
        # t4 = time.monotonic()
        ...
        # align vacuum obs
        vacuum_obs_timestamps = last_timestamp - (
                np.arange(self.gripper_obs_horizon)[::-1] * self.gripper_down_sample_steps * dt)
        for robot_idx, last_vacuum_data in enumerate(last_vacuums_data):
            # align vacuum obs
            vacuum_interpolator = get_step_interp1d(
                t=last_vacuum_data['vacuum_timestamp'],
                x=last_vacuum_data['vacuum_status'][..., None]
            )
            vacuum_obs = {
                f'robot{robot_idx}_vacuum_status': vacuum_interpolator(vacuum_obs_timestamps)
            }

            # update obs_data
            obs_data.update(vacuum_obs)
        ...
        if self.obs_accumulator is not None:
            ...
            for robot_idx, last_vacuum_data in enumerate(last_vacuums_data):
                self.obs_accumulator.put(
                    data={
                        f'robot{robot_idx}_vacuum_status': last_vacuums_data['vacuum_status'][...,None]
                    },
                    timestamps=last_vacuums_data['vacuum_timestamp']
                )
    ...
    def exec_actions(self, 
        ...
        for i in range(len(new_actions)):
            for robot_idx, (robot, gripper,vacuum, rc, gc, vc) in enumerate(zip(self.robots, self.grippers, self.vacuums, self.robots_config, self.grippers_config, self.vacuums_config)):
    ...
    def get_vacuum_state(self):
        return [vacuum.get_state() for vacuum in self.vacuums]
        ...
        if self.obs_accumulator is not None:
            ...
            if n_steps > 0:
                ...
                for robot_idx in range(len(self.robots)):
                    ...
                    episode[f'robot{robot_idx}_vacuum_status'] = gripper_interpolator(timestamps)
                    vacuum_interpolator = get_step_interp1d(
                        t=np.array(self.obs_accumulator.timestamps[f'robot{robot_idx}_vacuum_status']),
                        x=np.array(self.obs_accumulator.data[f'robot{robot_idx}_vacuum_status'])
                    )
                    episode[f'robot{robot_idx}_vacuum_status'] = vacuum_interpolator(timestamps)
